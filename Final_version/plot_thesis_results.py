...
def draw_heatmap(
    ...
    image = ax.imshow(np.ma.masked_invalid(values), cmap=cmap, vmin=vmin, vmax=vmax, aspect="auto")
...
def plot_reconstruction_baselines(
    ...
    for ax, dataset in zip(axes, datasets):
        ...
        llm_values = [safe_float(row.get("f1")) for row in llm_rows]
        llm_values = [value for value in llm_values if math.isfinite(value)]
        values = [max(llm_values) if llm_values else math.nan] + baseline_values
